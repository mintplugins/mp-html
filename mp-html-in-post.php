<?php
/*
Plugin Name: Move Plugins - HTML in Post
Plugin URI: http://moveplugins.com
Description: Put HTML chunks in a post
Version: 1.0
Author: Phil Johnston
Author URI: http://moveplugins.com
Text Domain: mp_html_in_post
...
*/
// Plugin version
if( !defined( 'MP_HTML_IN_POST_VERSION' ) )
	define( 'MP_HTML_IN_POST_VERSION', '1.0.0.0' );

// Plugin Folder URL
if( !defined( 'MP_HTML_IN_POST_PLUGIN_URL' ) )
	define( 'MP_HTML_IN_POST_PLUGIN_URL', plugin_dir_url( __FILE__ ) );

// Plugin Folder Path
if( !defined( 'MP_HTML_IN_POST_PLUGIN_DIR' ) )
	define( 'MP_HTML_IN_POST_PLUGIN_DIR', plugin_dir_path( __FILE__ ) );

// Plugin Root File
if( !defined( 'MP_HTML_IN_POST_PLUGIN_FILE' ) )
	define( 'MP_HTML_IN_POST_PLUGIN_FILE', __FILE__ );
...
function mp_html_in_post_textdomain() {

	// Set filter for plugin's languages directory
	$mp_html_in_post_lang_dir = dirname( plugin_basename( MP_HTML_IN_POST_PLUGIN_FILE ) ) . '/languages/';
	$mp_html_in_post_lang_dir = apply_filters( 'mp_html_in_post_languages_directory', $mp_html_in_post_lang_dir );
	...
	if ( file_exists( $mofile_global ) ) {
		...
		load_textdomain( 'mp_html_in_post', $mofile_global );
	} elseif ( file_exists( $mofile_local ) ) {
		// Look in local /wp-content/plugins/message_bar/languages/ folder
		load_textdomain( 'mp_html_in_post', $mofile_local );
	} else {
		// Load the default language files
		load_plugin_textdomain( 'mp_html_in_post', false, $mp_html_in_post_lang_dir );
	}

}
add_action( 'init', 'mp_html_in_post_textdomain', 1 );
